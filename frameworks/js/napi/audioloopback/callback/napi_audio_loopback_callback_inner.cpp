 ...
 */
#undef LOG_TAG
#define LOG_TAG "NapiAudioLoopbackCallbackInner"
...
void NapiAudioLoopbackCallbackInner::SaveCallbackReferenceInner(const std::string &callbackName, napi_value args,
    std::function<void(std::shared_ptr<AutoRef> generatedCallback)> successed)
{
    CHECK_AND_RETURN_LOG(CheckIfTargetCallbackName(callbackName),
        "NapiAudioLoopbackCallbackInner-> SaveCallbackReferenceInner UnKnown callback type: %{public}s",
        callbackName.c_str());
    napi_ref callback = nullptr;
    std::string taskName = "NapiAudioLoopbackCallbackInner::destroy";
...
bool NapiAudioLoopbackCallbackInner::ContainSameJsCallbackInner(const std::string &callbackName, napi_value args)
{
    CHECK_AND_RETURN_RET_LOG(CheckIfTargetCallbackName(callbackName), false,
        "NapiAudioLoopbackCallbackInner->ContainSameJsCallbackInner Unknown callback type: %{public}s",
        callbackName.c_str());
...
void NapiAudioLoopbackCallbackInner::RemoveCallbackReferenceInner(
    const std::string &callbackName, napi_env env, napi_value callback, std::function<void()> successed)
{
    ...
    CHECK_AND_RETURN_LOG(ContainSameJsCallbackInner(callbackName, callback),
        "NapiAudioRendererCallbackInner->RemoveCallbackReferenceInner wrong callback to remove %{public}s",
        callbackName.c_str());
    callbackCur = nullptr;
    if (successed != nullptr) {
        successed();
