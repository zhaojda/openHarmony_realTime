 ...
 */
#ifndef TAIHE_AUDIO_SESSION_DEVICE_CALLBACK_H
#define TAIHE_AUDIO_SESSION_DEVICE_CALLBACK_H
...
class TaiheAudioSessionDeviceCallback : public OHOS::AudioStandard::AudioSessionCurrentDeviceChangedCallback,
    public std::enable_shared_from_this<TaiheAudioSessionDeviceCallback> {
public:
    explicit TaiheAudioSessionDeviceCallback();
...
private:
    struct AudioSessionDeviceJsCallback {
        ...
        OHOS::AudioStandard::CurrentOutputDeviceChangedEvent audioSessionDeviceEvent;
    };

    void OnJsCallbackAudioSessionDevice(std::unique_ptr<AudioSessionDeviceJsCallback> &jsCb);
    static void SafeJsCallbackAudioSessionDeviceWork(AudioSessionDeviceJsCallback *event);
